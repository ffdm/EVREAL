...
def evaluate2(method_names, eval_config_names=None, dataset_names=None, metrics=None):
    """
    Evaluate the given methods on the given datasets with the given evaluation configs and metrics.
    :param method_names: list of method names to evaluate. Each has a corresponding config file in config/method.
        Available methods are E2VID, E2VID+, FireNet, FireNet+, SPADE-E2VID, SSL-E2VID, ET-Net, and HyperE2VID.
    ...
    eval_config = get_eval_configs(eval_config_names)[0]

    dataset_configs = get_dataset_configs(dataset_names)

    
    # eval_method_with_config(eval_config, method_name, datasets, metrics):

    # num_sequences = get_number_of_sequences_in_all_datasets(datasets)
    
    method_name = 'HyperE2VID'
    method_config = get_method_config(method_name)
    ...
    f = h5py.File('/data1/fdm/eTraM/Static/HDF5/train_h5_1/train_day_0001_td.h5', 'r')
    events = f['events']

    # This guy has 180s of events. I want 20fps
    # Every frame is 50000 us. Therefore there is 3600 total frames
...
if __name__ == '__main__':
    ...
    evaluate2(args.method, args.config, args.dataset, args.metrics)
